...
# Optional extras — per-instance identifiers kept out of git. Empty default =
# section omitted from config.yml → Ansible falls back to role/group defaults.
: "${HOMEPAGE_TITLE:=Homelab}"
: "${HOMEPAGE_REPO_URL:=}"
: "${HOMEPAGE_EXT_UPTIME_URL:=}"
: "${HOMEPAGE_EXT_UPTIME_SLUG:=homelab}"
: "${HOMEPAGE_EXT_CAPROVER_URL:=}"
: "${HOMEPAGE_EXT_WG_HUB_IP:=}"
: "${UPTIME_HTTP_TARGET:=}"
: "${UPTIME_ICMP_TARGET:=}"
...
# Optional blocks — appended only when relevant env vars are set, so the repo
# defaults (empty list) stay authoritative when extras aren't configured.
{
  if [ -n "$HOMEPAGE_REPO_URL" ]; then
    cat <<YAML
...
YAML
  ...
  if [ -n "$HOMEPAGE_EXT_UPTIME_URL" ] || [ -n "$HOMEPAGE_EXT_CAPROVER_URL" ]; then
    printf '\nhomepage_services_extra:\n  - group: Hetzner\n    items:\n'
    if [ -n "$HOMEPAGE_EXT_UPTIME_URL" ]; then
      cat <<YAML
      - name: Uptime Kuma
        href: "${HOMEPAGE_EXT_UPTIME_URL}"
        icon: uptime-kuma.svg
        description: Reverse-uptime probes
        siteMonitor: "${HOMEPAGE_EXT_UPTIME_URL}"
        widget:
          type: uptimekuma
          url: "${HOMEPAGE_EXT_UPTIME_URL}"
          slug: "${HOMEPAGE_EXT_UPTIME_SLUG}"
YAML
    fi
    if [ -n "$HOMEPAGE_EXT_CAPROVER_URL" ]; then
      cat <<YAML
      - name: CapRover
        href: "${HOMEPAGE_EXT_CAPROVER_URL}"
        icon: caprover.png
        description: PaaS control panel
YAML
    fi
    if [ -n "$HOMEPAGE_EXT_WG_HUB_IP" ] && [ -n "$HOMEPAGE_EXT_CAPROVER_URL" ]; then
      cat <<YAML
      - name: WG Hub
        href: "${HOMEPAGE_EXT_CAPROVER_URL}"
        icon: wireguard.svg
        description: "Upstream VPN concentrator · ${HOMEPAGE_EXT_WG_HUB_IP}"
        ping: "${HOMEPAGE_EXT_WG_HUB_IP}"
YAML
    fi
  fi

  if [ -n "$UPTIME_HTTP_TARGET" ]; then
    printf '\nobservability_uptime_http_targets:\n  - "%s"\n' "$UPTIME_HTTP_TARGET"
  fi
  if [ -n "$UPTIME_ICMP_TARGET" ]; then
    printf '\nobservability_uptime_icmp_targets:\n  - "%s"\n' "$UPTIME_ICMP_TARGET"
  fi
} >> /etc/homelab/config.yml
...
gen_secret_if_missing ntfy_bridge_webhook_password 32

ok "/etc/homelab/ populated"

### ---------- phase 7: GitHub Actions runner (ephemeral JIT, N instances) ----------

step "Phase 7 — installing ${GITHUB_RUNNER_INSTANCES}× ephemeral JIT self-hosted GitHub Actions runner(s)"
...
# Persistent host-local CI cache. $RUNNER_TEMP is wiped per-job, so pip,
# ansible-galaxy, and fact-cache artefacts would re-download every run without
# this. CI jobs point PIP_CACHE_DIR, ANSIBLE_COLLECTIONS_PATH and
# ANSIBLE_CACHE_PLUGIN_CONNECTION here. flock in the composite action
# serialises the first-time galaxy install across parallel deploy-* jobs
# (concurrent galaxy on an empty tree raced on MIT.txt).
install -d -o "$GITHUB_RUNNER_USER" -g "$GITHUB_RUNNER_USER" -m 0750 \
  /opt/gha-cache \
  /opt/gha-cache/pip \
  /opt/gha-cache/ansible-collections \
  /opt/gha-cache/ansible-facts
...
# Per-instance install dirs (${GITHUB_RUNNER_DIR}-1, -2, …). Parallel jobs need
# separate _work trees + separate _diag; one install dir per instance is the
# cleanest isolation and matches the actions/runner model.
for i in $(seq 1 "$GITHUB_RUNNER_INSTANCES"); do
...
# installdependencies.sh is identical across instances — run once.
first_dir="${GITHUB_RUNNER_DIR}-1"
[ -x "$first_dir/bin/installdependencies.sh" ] && \
  "$first_dir/bin/installdependencies.sh" >/dev/null || true
...
# RUNNER_DIR is injected by the systemd template unit (gha-runner-jit@.service).
set -euo pipefail
umask 077

CONFIG=/etc/homelab/config.yml
PAT_FILE=/etc/homelab/secrets/github_pat
: "${RUNNER_DIR:?RUNNER_DIR must be set by systemd Environment=}"
...
# Include install-dir basename so runner names on the GH side disambiguate
# which instance handled a given job.
name="$(hostname -s)-$(basename "$RUNNER_DIR")-$(date +%s)-$$"
...
[Unit]
Description=GitHub Actions ephemeral JIT runner (instance %i)
...
Environment=RUNNER_DIR=${GITHUB_RUNNER_DIR}-%i
WorkingDirectory=${GITHUB_RUNNER_DIR}-%i
...
# Migrate from non-templated single-instance unit to @N template.
if [ -e /etc/systemd/system/gha-runner-jit.service ]; then
  note "migrating legacy gha-runner-jit.service → gha-runner-jit@.service"
  systemctl stop gha-runner-jit.service 2>/dev/null || true
  systemctl disable gha-runner-jit.service 2>/dev/null || true
  rm -f /etc/systemd/system/gha-runner-jit.service
fi

# Drop the unsuffixed install dir from the old single-instance layout.
if [ -d "$GITHUB_RUNNER_DIR" ] && [ -x "$GITHUB_RUNNER_DIR/run.sh" ]; then
  note "removing legacy install dir $GITHUB_RUNNER_DIR (replaced by ${GITHUB_RUNNER_DIR}-N)"
  rm -rf "$GITHUB_RUNNER_DIR"
fi

# Downscale: stop + remove any @N instance beyond the configured count.
for u in $(systemctl list-unit-files --type=service --no-legend 2>/dev/null | awk '/^gha-runner-jit@[0-9]+\.service/ {print $1}'); do
  n=$(echo "$u" | sed -E 's/^gha-runner-jit@([0-9]+)\.service$/\1/')
  if [ "$n" -gt "$GITHUB_RUNNER_INSTANCES" ]; then
    note "downscaling: stopping + removing $u"
    systemctl stop "$u" 2>/dev/null || true
    systemctl disable "$u" 2>/dev/null || true
...
ok "${GITHUB_RUNNER_INSTANCES}× ephemeral JIT runner service(s) active"
